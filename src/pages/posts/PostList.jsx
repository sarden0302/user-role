...
const PostList = () => {
    ...
    const [keyword, setKeyword] = useState(""); // keyword 는 단순히 검색어이기 때문에 배열 xxx
    ...
    const handleChange = (e) => {   // input 에 어떤 이벤트가 발생할 경우 작동하는 기능
        const value = e.target.value.trim(); // 이벤트가 발생한 input 에 들어있는 값을 공백 제거하고 value 변수 이름에 대입
        setKeyword(value);

        if (value) {
            apiService.suggestedPosts(value,  setPosts,  setErr);
        }
    ...
    function handleSug(sug) {
        ...
        setShow(false);// 더이상 추천 리스트를 보여줄 필요가 없기 때문에 숨기기 설정
    }
    ...
    function handleSearch() {
        ...
        apiService.searchPost(keyword, setPosts, setErr);
    }

    /*
        onBlur = 입력 필드에서 포커스가 벗어날 때 실행되는 이벤트
                 input 에서 작성하던 중 마우스가 input 태그가 아닌 다른 곳을 클릭했을 경우
                 200(0.2초) 후에 키워드에 따른 추천 검색 리스트 제안을 종료(setTimeout)
                ex) onBlur={() => setTimeout(() => setShow(false), 200)}

        onMouseDown = 마우스를 클릭하는 순간 실행되는 이벤트(동작, 행위)
                      사용자가 추천 검색어를 클릭할 경우 추천 검색어 리스트가 onBlur 로 인해 사라지고
                      마우스 클릭을 누르는 순간 제안한 리스트 중에서 사용자가 선택한 키워드 내부로 들어갈
                      수 있도록 설정
                    ex) onMouseDown={ () => handleSug(sug)}
